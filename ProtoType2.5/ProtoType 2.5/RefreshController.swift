...
class RefreshController: UIViewController {
    ...
    @objc func countdonwAction(){
        if(time == 0){
            ...
            if (sessionOver == true){
                ...
                self.performSegue(withIdentifier: "RefreshtoQuestion", sender: self)
            }
                ...
                let questions = try JSONDecoder().decode([Question].self, from: data)
                    ...
                    for item in questions {
                        ...
                        if(item.QID!==100){
                            ...
                            self.time = item.time!
                            self.answerIntRefresh = item.Answer!
                        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "RefreshtoQuestion" {
            if let destinationVC = segue.destination as? QuestionController {
                //Send the content to resultController
                destinationVC.content = questionString
                destinationVC.timeCount = self.time
                destinationVC.answerInt = self.answerIntRefresh
            }
        }
    }
